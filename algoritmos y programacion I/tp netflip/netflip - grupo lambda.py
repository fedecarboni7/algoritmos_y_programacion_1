...
def titulo_submenu(texto):
    print(
        f'\n{Fore.LIGHTCYAN_EX}{Back.BLUE}{texto.center(80, "-")}{Style.RESET_ALL}\n')


def titulo_opcion(texto):
    print(f'\n{Fore.LIGHTCYAN_EX}{Back.CYAN}{texto.center(80, "-")}{Style.RESET_ALL}')


def ingresar_opcion():
    opcion = input(
        f"\n{Fore.LIGHTCYAN_EX}Ingrese una opción:{Style.RESET_ALL} ")
    return opcion
...
def merge_usuarios():
    ...
    try:
        ...
        return mostrar_error("No hay archivos de usuarios suficientes para generar un merge.\nPor favor elija otra opción.", True)
    ...
    if errores:
        mostrar_error(
            'Se encontraron errores en la carga de usuarios, revise "log.txt" para comprobar los usuarios afectados.', True)
    pantalla_en_espera(accion_exitosa(
...
def alta_de_usuario():
    ...
    with open(archivo_usuarios, 'a') as usuarios:
        ...
        while seguir == "s":
            ...
            nombre = ingresar_datos("Ingrese nombre: ")
            if nombre[0].islower():
                nombre = nombre.capitalize()
            apellido = ingresar_datos("Ingrese apellido: ")
            if apellido[0].islower():
                apellido = apellido.capitalize()
            año_de_nacimiento = ingresar_datos("Ingrese año de nacimiento: ")
            ...
            accion_exitosa(f'Nombre y Apellido: {nombre} {apellido}')
            accion_exitosa(f'Año de nacimiento: {año_de_nacimiento}')

            seguir = ingresar_datos(
...
def baja_de_usuario():
    ...
    id_baja = ingresar_datos('Ingrese el ID de usuario: ')
    ...
    if baja_exitosa:
        ...
        mostrar_error('El usuario {id_baja} no existe en la base de datos.\n')
    pantalla_en_espera()
    return
...
def alta_de_pelicula():
    ...
    while seguir == "s":
        ...
        director_nombre = ingresar_datos("Ingrese nombre del director: ")
        if director_nombre[0].islower():
            director_nombre = director_nombre.capitalize()
        director_apellido = ingresar_datos("Ingrese apellido del director: ")
        ...
        print("5. Acción")
        print("6. Romantica")
        opcion = int(ingresar_datos("\nIngrese genero: "))
        tupla_genero = (opcion, generos[opcion])
        puntaje = int(ingresar_datos("Ingrese puntaje del 1 al 9: "))
        vistas = 0
...
def baja_de_pelicula():
    ...
    id_baja = ingresar_datos('Ingrese el ID de pelicula: ')
    baja_exitosa = False
...
def listar_pelicula_puntaje():
    ...
    print('ID Pelicula'.ljust(15) + 'Nombre'.ljust(max(len(max_nombre) + 3, 8)) + 'Director'.ljust(
        max(len(max_director) + 3, 10)) + 'Género'.ljust(16) + 'Puntaje' + Style.RESET_ALL + '\n')
    for pelicula in lista_peliculas:
...
def listar_pelicula_gen():
    ...
    while indice_peliculas < len(lista_peliculas):
        print(Fore.LIGHTCYAN_EX + ''.center(40 + max(len(max_nombre) +
        ...
        print(Fore.LIGHTBLUE_EX + f' Género: {genero} '.center(40 + max(
        ...
        while genero == genero_anterior:
            ...
            print(Fore.LIGHTBLUE_EX + 'Director:', (' ' + director).rjust(30 +
                                                                          max(len(max_nombre) + 2, 8) + max(len(max_director) + 2, 10), '-') + Style.RESET_ALL)
            print('Nombre'.ljust(max(len(max_nombre) + 2, 18)),
                  'ID Pelicula'.ljust(24), 'Puntaje')
            while director == director_anterior:
                print(nombre_pelicula.ljust(max(len(max_nombre) + 2, 18)),
                      id_pelicula.ljust(24), puntaje)
                indice_peliculas += 1
...
def asignar_pelicula_usuario():
    ...
    titulo = ingresar_datos('Ingrese el nombre de la película: ')
    for pelicula in lista_peliculas:
        if titulo in pelicula:
            ...
            if opcion == '1':
                ...
                usuario = str(ingresar_datos('Ingrese el ID del usuario: '))
...
def cinco_mas_vistas():
    ...
    print(''.center(40 + max(len(max_nombre) + 2, 8) +
                    max(len(max_director) + 2, 10), '-'))
    print('ID Pelicula'.ljust(21) + 'Nombre'.ljust(max(len(max_nombre) + 3, 18)) +
          'Director'.ljust(max(len(max_director) + 3, 20)) + 'Puntaje' + Style.RESET_ALL + '\n')
    for i in range(0, min(len(lista_peliculas), 5)):
        id_pelicula, nombre_pelicula, director, genero, puntaje, vistas = lista_peliculas[i]
        print(id_pelicula.ljust(20), nombre_pelicula.ljust(max(len(
            max_nombre) + 2, 18)), director.ljust(max(len(max_director) + 2, 20)), puntaje)
    pantalla_en_espera()
    return
...
def buscar_usuario():
    ...
    print('1. Buscar usuario por nombre y apellido')
    print('2. Buscar usuario por ID')
    opcion = ingresar_datos('\nIngrese una opción: ')
    if opcion == '1':
        usuario = ingresar_datos('Ingrese el nombre y apellido del usuario: ')
...
def recomendacion_ultima_pelicula():
    ...
    if nombre != None:
        ...
        # Imprimir resultado
        accion_exitosa(
            f'Última pelicula vista por {nombre}: {ultima_pelicula_vista}')
        accion_exitosa(
